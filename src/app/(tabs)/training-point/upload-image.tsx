...
export default function UploadImage() {
    ...
    // const getFileSize = async (uri: string): Promise<number | string> => {
    //     try {
    //         const response = await fetch(uri);
    //         const blob = await response.blob();
    //         return (blob.size / 1024).toFixed(2); // Dung lượng (KB)
    //     } catch (error) {
            ...
            const manipulatedImages = await Promise.all(
                result.assets.map(async (asset: any) => {
                    ...
                    const manipulatedImage = await ImageManipulator.manipulateAsync(
                        ...
                        { compress: 0.5, format: ImageManipulator.SaveFormat.JPEG }, // Nén và chuyển sang JPEG
                    );
